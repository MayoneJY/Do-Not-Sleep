...
앱을 처음 설치한 뒤에는 아래 순서로 한 번만 설정하면 됩니다.

1. `Do Not Sleep.app`을 엽니다.
2. 메뉴 막대의 초승달 아이콘을 눌러 `관리자 권한 적용/갱신`을 선택합니다.
3. macOS 관리자 암호를 입력합니다. 설정 중에는 메뉴 막대 아이콘 자리에 spinner가 표시됩니다.
4. macOS가 앱 수정 시도를 차단했다는 알림을 띄우면 `시스템 설정 > 개인정보 보호 및 보안 > 앱 관리`에서 Do Not Sleep을 허용한 뒤 `관리자 권한 적용/갱신`을 다시 실행합니다.
5. 설정 완료 후 Codex 훅 신뢰 안내가 뜨면 `Codex 열기`를 누릅니다.
6. Codex에서 `/hooks`를 열고 `Do Not Sleep 훅 등록 세션 동기화` command hook을 신뢰 처리합니다.
7. 이후에는 Codex 작업이 시작되면 자동으로 잠자기 방지가 켜지고, 마지막 세션이 끝나면 자동으로 해제됩니다.

이 과정에서 앱은 관리자 권한 helper를 설치하고 Codex/Claude Code 훅을 등록합니다. Python이나 별도 터미널 명령은 필요하지 않습니다.
